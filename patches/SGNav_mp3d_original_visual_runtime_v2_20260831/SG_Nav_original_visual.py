...
from scenegraph_original_visual import SceneGraph
...
class SG_Nav_Agent():
    def __init__(self, task_config, args=None):
        ...
        # Keep output filenames aligned with the selected dataset episode index.
        self.count_episodes = self.args.episode_start - 1
        self.loop_time = 0
        ...
        # Never overwrite videos produced by the untouched paper entry point.
        self.experiment_name = 'experiment_0_original_visual'
    ...
    def visualize(self, traversible, observations, number_action):
        if self.args.visualize:
            # Read state that the original navigation pipeline has already
            # produced.  This does not run an additional detector, LLM or VLM.
            self.text_node, self.text_edge, self.explanation = (
                self.scenegraph.get_visualization_text()
            )
...
def main():
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="Zero-based episode index after applying the scene split.",
    )
    parser.add_argument(
        "--num_episodes",
        default=None,
        type=int,
        help="Number of consecutive episodes to run from --episode_start.",
    )
    args = parser.parse_args()
    if args.episode_start < 0:
        parser.error("--episode_start must be greater than or equal to 0")
    if args.num_episodes is not None and args.num_episodes <= 0:
        parser.error("--num_episodes must be greater than 0")
    ...
    all_episodes = list(challenge._env.episodes)
    if args.episode_start >= len(all_episodes):
        parser.error(
            "--episode_start {} is outside the selected episode list "
            "(length {})".format(args.episode_start, len(all_episodes))
        )

    if args.episode_start == 0 and args.num_episodes is None:
        challenge.submit(agent)
    else:
        episode_end = (
            len(all_episodes)
            if args.num_episodes is None
            else min(len(all_episodes), args.episode_start + args.num_episodes)
        )
        selected_episodes = all_episodes[args.episode_start:episode_end]
        challenge._env.episodes = selected_episodes
        selected_ids = [episode.episode_id for episode in selected_episodes]
        print(
            "[SG-Nav Selection] episode indexes [{}:{}) ids={}".format(
                args.episode_start, episode_end, selected_ids
            ),
            flush=True,
        )
        metrics = challenge.evaluate(agent, num_episodes=len(selected_episodes))
        for key, value in metrics.items():
            habitat.logger.info("{}: {}".format(key, value))
