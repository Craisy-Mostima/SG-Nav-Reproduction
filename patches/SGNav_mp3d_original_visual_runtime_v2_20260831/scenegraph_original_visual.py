...
class SceneGraph():
    def __init__(self, map_resolution, map_size_cm, map_size, camera_matrix, is_navigation=True, agent=None) -> None:
        ...
        self._visualization_cache_step = -100
        self._visualization_cache_llm_call = -1
...
Object pair(s):
    ...
    @staticmethod
    def _compact_visualization_text(text, max_chars):
        """Keep text readable inside the fixed-size paper video panels."""
        compact = ' '.join(str(text).split())
        if len(compact) > max_chars:
            return compact[:max_chars - 3] + '...'
        return compact

    def _record_model_response(self, kind, response):
        """Record a response the original algorithm already requested."""
        self.llm_call_count += 1
        self.last_llm_kind = kind
        self.last_llm_response = str(response)
        response_text = self._compact_visualization_text(response, 210)
        self.reason_visualization = (
            f'{kind} call #{self.llm_call_count}: {response_text}'
        )

    def get_visualization_text(self):
        """Return a lightweight snapshot without changing navigation state.

        The original entry point draws three text panels but never updates
        their strings.  We refresh this display at most once every five
        navigation steps, or immediately after an existing LLM/VLM response.
        No model call is made here.
        """
        step = getattr(self.agent, 'navigate_steps', 0)
        llm_changed = self.llm_call_count != self._visualization_cache_llm_call
        if (
            self._visualization_cache_step >= 0
            and step - self._visualization_cache_step < 5
            and not llm_changed
        ):
        ...
        node_items = []
        for node in self.nodes:
            caption = node.caption or 'unknown'
            room = getattr(getattr(node, 'room_node', None), 'caption', 'unknown room')
            goal_mark = ' [GOAL]' if getattr(node, 'is_goal_node', False) else ''
            node_items.append(f'{caption} ({room}){goal_mark}')
        node_items.sort()

        valid_edges = set()
        for node in self.nodes:
            valid_edges.update(
                edge for edge in node.edges if getattr(edge, 'relation', None)
            )
        edge_items = sorted(
        ...
        if node_items:
            node_text = f'Nodes={len(node_items)}: ' + '; '.join(node_items)
        else:
            node_text = (
                'Nodes=0 (scene graph is still building; '
                f'2D detections={len(self.segment2d_results)}, '
                f'3D objects={len(self.objects_post)})'
            )

        if edge_items:
            edge_text = f'Edges={len(edge_items)}: ' + '; '.join(edge_items)
        else:
            edge_text = f'Edges=0 (nodes={len(node_items)})'

        explanation = self.reason_visualization or (
            'No LLM/VLM call yet. The scene graph may still be building, '
            'or GLIP is handling the detected goal directly.'
        )
        self.edge_text = self._compact_visualization_text(edge_text, 220)
        self._visualization_cache = (
            self._compact_visualization_text(node_text, 220),
            self.edge_text,
            self._compact_visualization_text(explanation, 280),
        )
        self._visualization_cache_step = step
        self._visualization_cache_llm_call = self.llm_call_count
        return self._visualization_cache
    ...
    def get_llm_response(self, prompt):
        ...
        self._record_model_response('LLM', content)
        return content
    ...
    def get_vlm_response(self, prompt, image):
        ...
        content = response.message.content
        self._record_model_response('VLM', content)
        return content
