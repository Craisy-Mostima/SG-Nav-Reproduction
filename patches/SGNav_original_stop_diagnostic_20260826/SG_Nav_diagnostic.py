...
class SG_Nav_Agent():
    ...
    def _diag(self, event, **details):
        """Emit read-only diagnostics without changing navigation decisions."""
        episode = None
        scene = None
        if hasattr(self, "simulator") and getattr(self.simulator, "_env", None) is not None:
            current_episode = getattr(self.simulator._env, "current_episode", None)
            if current_episode is not None:
                episode = getattr(current_episode, "episode_id", None)
                scene = getattr(current_episode, "scene_id", None)

        payload = {
            "event": event,
            "episode": episode,
            "scene": scene,
            "goal": getattr(self, "obj_goal", None),
            "total_steps": getattr(self, "total_steps", None),
            "navigate_steps": getattr(self, "navigate_steps", None),
            "found_goal": getattr(self, "found_goal", None),
            "found_possible_goal": getattr(self, "found_possible_goal", None),
            "found_goal_times": getattr(self, "found_goal_times", None),
            "not_move_steps": getattr(self, "not_move_steps", None),
            "using_random_goal": getattr(self, "using_random_goal", None),
            "loop_time": getattr(self, "loop_time", None),
        }
        ...
        print(
            "[SG-Nav Diagnostic] " + json.dumps(payload, default=json_default, ensure_ascii=False),
            flush=True,
        )

    def _diag_target_detection(self):
        """Report GLIP target matches and their observed center-pixel depths."""
        labels = list(self.current_obj_predictions.get_field("labels"))
        scores = self.current_obj_predictions.get_field("scores")
        boxes = self.current_obj_predictions.bbox
        matches = []
        for index, label in enumerate(labels):
            is_target = self.obj_goal in label
            if self.obj_goal == "gym_equipment":
                is_target = is_target or label in ["treadmill", "exercise machine"]
            if not is_target:
                continue
            box = boxes[index].detach().cpu().to(torch.int64)
            center = ((box[:2] + box[2:]) // 2).tolist()
            center_x = max(0, min(int(center[0]), self.depth.shape[1] - 1))
            center_y = max(0, min(int(center[1]), self.depth.shape[0] - 1))
            matches.append(
                {
                    "label": label,
                    "score": float(scores[index].detach().cpu()),
                    "bbox": box.tolist(),
                    "center_depth_m": float(self.depth[center_y, center_x, 0]),
                }
            )
        self._diag("target_detection", target_matches=matches)
    ...
    def act(self, observations):
        if self.total_steps >= 500:
            self._diag("return_stop", reason="max_steps_500")
            return {"action": 0}
        ...
        if number_action == 0:
            self._diag(
                "planner_zero_action_initial",
                replan=bool(replan),
                planner=getattr(self, "_diag_last_plan", None),
            )
        ...
        if (not self.found_goal and not self.found_possible_goal and number_action == 0) or (self.using_random_goal and self.move_since_random > 20): 
            ...
            if number_action == 0:
                self._diag(
                    "planner_zero_action_after_frontier_refresh",
                    replan=bool(replan),
                    planner=getattr(self, "_diag_last_plan", None),
                )
        ...
        while (not self.found_goal and number_action == 0) or self.not_move_steps >= 7:
            ...
            if self.loop_time > 20:
                self._diag(
                    "return_stop",
                    reason="random_goal_retry_limit_20",
                    replan=bool(replan),
                    planner=getattr(self, "_diag_last_plan", None),
                )
                return {"action": 0}
        ...
        if number_action == 0:
            reason = "planner_stop_after_target_found" if self.found_goal else "planner_returned_stop"
            self._diag(
                "return_stop",
                reason=reason,
                replan=bool(replan),
    ...
    def _get_stg(self, traversible, start, goal, goal_found):
        ...
        self._diag_last_plan = {
            "planner_stop": bool(stop),
            "replan": bool(replan),
            "goal_found": bool(goal_found),
            "state_grid": [int(state[0]), int(state[1])],
            "short_term_goal_grid": [float(stg_y), float(stg_x)],
            "raw_fmm_distance_grid": float(self.planner.fmm_dist[state[0], state[1]]),
            "dilation_deg": int(self.dilation_deg),
            "decrease_stop_cond": float(decrease_stop_cond),
        }
    ...
    def update_metrics(self, metrics):
        ...
        if self.simulator._env.episode_over:
            self._diag("episode_over", metrics=dict(self.metrics))
